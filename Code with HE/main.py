...
for u in user_id_list:
    user_list.append(User(u,batch_size,randrange(40,65),data_path,lr))
...
for i in range(num_iters):
    server_train_acc,aggregated_weights = server.train()
    print(f"\nServer train accuracy at iteration {i+1} = {server_train_acc}")
    if i>0:
        if server_train_acc > accuracies[-1]:
            #server.model.initialise_parameters(aggregated_weights)
            print(f"Accuracy improved from {accuracies[-1]} to {server_train_acc}")
            count = 0
        else:
            count += 1
    ...
    accuracies.append(server_train_acc)
